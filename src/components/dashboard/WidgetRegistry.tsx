...
const RevenueZettleWidget = ({ data }: { data: DashboardData }) => {
  ...
  if (!rev) return <div className="h-full flex items-center justify-center text-slate-400 text-xs bg-slate-50 border border-slate-100 rounded-lg">Sin acceso financiero</div>;
  ...
  return (
    <Card className="h-full shadow-sm border-slate-200 flex flex-col justify-center bg-white">
      <CardContent className="p-5 flex flex-col justify-between h-full">
        <div>
          <div className="flex justify-between items-start mb-2"><p className="text-[10px] uppercase tracking-wider font-bold text-slate-400">Facturación Diaria</p><div className="bg-slate-100 p-1.5 rounded-full"><Banknote size={14} className="text-slate-600"/></div></div>
        ...
        <div className={cn("flex items-center text-xs font-medium mt-4", isPos ? "text-emerald-600" : "text-red-500")}>
            <TrendingUp size={14} className={cn("mr-1", !isPos && "rotate-180")} />{Math.abs(rev.vsYesterday).toFixed(1)}% vs ayer
        </div>
      </CardContent>
    </Card>
  );
    ...
    return (
        <Card className="h-full shadow-sm border-slate-200 flex flex-col bg-white overflow-hidden min-h-[300px]">
            <CardHeader className="p-4 py-3 border-b border-slate-50 bg-slate-50/50 flex flex-row items-center justify-between shrink-0">
                <div className="flex items-center gap-2"><CalendarCheck size={16} className="text-slate-500"/><CardTitle className="text-sm font-bold text-slate-900">Agenda del Día</CardTitle></div>
                <Badge variant="secondary" className="font-mono text-[10px] bg-white border border-slate-200">{items.length}</Badge>
            </CardHeader>
                        ...
                        {sortedItems.map((appt) => {
                            ...
                            return (
                                <div key={appt.id} className={cn("flex items-center gap-3 p-3 hover:bg-slate-50 transition-colors", isDone && "opacity-50 grayscale")}>
                                    <div className="flex flex-col items-center justify-center w-12 shrink-0 border-r border-slate-100 pr-2"><span className="text-xs font-bold text-slate-700">{format(date, 'HH:mm')}</span><span className="text-[9px] text-slate-400 font-medium uppercase">{format(date, 'a', { locale: es })}</span></div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between"><p className="text-xs font-bold text-slate-800 truncate">{appt.pet_name}</p>{isDone && <CheckCircle2 size={12} className="text-emerald-500"/>}</div>
                                        <div className="flex items-center gap-2 mt-1"><Badge variant="outline" className={cn("text-[9px] h-4 px-1 rounded border-0 font-normal", isCut ? "bg-purple-50 text-purple-700" : "bg-cyan-50 text-cyan-700")}>{isCut ? <Scissors size={8} className="mr-1"/> : <Droplets size={8} className="mr-1"/>}<span className="truncate max-w-[140px]">{appt.service_name}</span></Badge></div>
                                    </div>
                                </div>
                            );
                        })}
                    </div>
                ) : <div className="flex flex-col items-center justify-center h-full text-slate-400 p-6 text-center gap-2 min-h-[200px]"><Calendar size={24} className="opacity-20"/><p className="text-xs">No hay citas hoy.</p></div>}
            </CardContent>
        </Card>
    );
};
...
const StaffStatusWidget = ({ data }: { data: DashboardData }) => {
    ...
    return (
        <Link href="/admin/team" className="contents">
            <Card className="h-full shadow-sm border-slate-200 flex flex-col bg-white overflow-hidden cursor-pointer hover:border-slate-300 transition-colors group">
                <CardHeader className="p-4 py-3 border-b border-slate-50 flex flex-row justify-between items-center bg-slate-50/30 group-hover:bg-slate-50"><CardTitle className="text-sm font-bold text-slate-800 group-hover:text-blue-600 transition-colors">Equipo (Admin)</CardTitle><Badge variant="secondary" className="text-[10px] h-5">{data.staff?.length || 0}</Badge></CardHeader>
...
const RetentionWidget = ({ data }: { data: DashboardData }) => {
    ...
    return (
        <Card className="h-full shadow-sm border-slate-200 flex flex-col bg-white overflow-hidden">
            <CardHeader className="p-4 py-3 flex flex-row items-center justify-between space-y-0 border-b border-slate-50"><CardTitle className="text-sm font-bold text-slate-800 flex items-center gap-2 truncate"><AlertTriangle className="h-4 w-4 text-amber-500 shrink-0"/> <span className="truncate">Watchlist</span></CardTitle><Badge variant="outline" className="text-[9px] h-5 px-1.5 font-bold text-blue-700 bg-blue-50 border-blue-200 whitespace-nowrap cursor-pointer hover:bg-blue-100">Recuperar</Badge></CardHeader>
            <CardContent className="p-0 flex-1 overflow-y-auto">
                {riskClients.length > 0 ? (
                    <div className="divide-y divide-slate-50">
                        {riskClients.map((client, i) => (
                            <div key={i} className="flex items-center justify-between p-3 group hover:bg-slate-50">
                                <div className="min-w-0 pr-2"><p className="text-xs font-semibold text-slate-700 truncate">{client.name}</p><p className="text-[10px] text-slate-400 flex items-center gap-1"><Clock size={10}/> {client.days_ago} días ausente</p></div><Button size="sm" variant="ghost" className="h-7 w-7 p-0 rounded-full text-blue-600 bg-blue-50 hover:bg-blue-100 shrink-0"><Phone size={14}/></Button>
                            </div>
                        ))}
                    </div>
...
// --- CATALOGO DEFINITIVO Y ROLES AJUSTADOS ---
export const WIDGET_CATALOG = {
  // FINANCIEROS Y ADMIN (Admin + Recepción para caja/watchlist)
