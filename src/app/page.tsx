...
// NOTA: Importamos el tipo solo para referencia, pero no lo forzamos en la constante para permitir campos extra
...
export default async function DashboardPage() {
  ...
  // Helpers Supabase (ISO)
  const todayStartSupabase = startOfDay(now).toISOString();
  const todayEndSupabase = endOfDay(now).toISOString();
  ...
  if (user) {
    ...
    if (settings?.dashboard_layout && Array.isArray(settings.dashboard_layout) && settings.dashboard_layout.length > 0) {
      ...
      // Default híbrido: Finanzas + Operaciones + Staff + Agenda
  ...
  // CORRECCIÓN: Eliminamos la anotación de tipo explícita para evitar errores de propiedades extra
  const dashboardData = {
    revenue: finance, 
    agenda: {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-50/50 overflow-y-auto">
      <div className="max-w-[1600px] mx-auto w-full p-4 md:p-8 space-y-6">
        ...
        {/* Usamos 'as any' en la prop data si DraggableDashboard es muy estricto, o confiamos en que TS acepte el objeto extendido */}
